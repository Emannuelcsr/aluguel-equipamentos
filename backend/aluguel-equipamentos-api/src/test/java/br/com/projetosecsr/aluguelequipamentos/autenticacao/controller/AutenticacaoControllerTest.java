...
@Import({ ConfiguracaoDeSeguranca.class, TratadorGlobalDeErros.class })
public class AutenticacaoControllerTest {
	...
	void deveRealizarLoginQuandoDadosForemValidos() throws Exception {
		...
		UsuarioAutenticadoResponse usuario = new UsuarioAutenticadoResponse(1L,
	            "Administrador",
	            "admin@empresa.com",
	            PerfilUsuario.ADMINISTRADOR);

	    LoginResponse respostaDoService = new LoginResponse(
	            "token-jwt-teste",
	            "Bearer",
	            expiraEm,
	            usuario
	    ...
	    when(autenticacaoService.autenticar(any(LoginRequest.class)))
	...
	void deveRetornarAcessoNegadoQuandoUsuarioEstiverInativo() throws Exception {
	    ...
	    mockMvc.perform(post("/api/autenticacao/login")
	            ...
	            .andExpect(jsonPath("$.mensagens[0]")
	                    .value("A conta do usuário está inativa."))
	            .andExpect(jsonPath("$.path").value("/api/autenticacao/login"))
